...
/** biome-ignore-all lint/correctness/noUnusedPrivateClassMembers: reason */
declare global {
...
export class PictureInPictureHandler {
	...
	static async createWindow(element: Element): Promise<void> {
		if (!PictureInPictureHandler.isSupported()) {
		...
		try {
			...
			try {
				...
				await PictureInPictureHandler.setupPipWindow(pipWindow, element);
			} finally {
			}
		} catch (error) {
	...
	private static setupPipWindow(pipWindow: Window, element: Element): void {
		...
		const { wrapper } = PictureInPictureHandler.cloneElementWithParents(
			element,
			pipWindow,
		);
		...
		PictureInPictureHandler.setupPipWindowStyles(pipWindow);
